...
function Company_Dashboard() {
  ...
  return (
    <div className='h-screen'>
      ...
      <div className='flex mx-auto py-4 sm:px-6 lg:px-8 '>
        <sidebar className=' w-1/4 bg-gray-800 mx-auto sm:px-6 lg:px-8 float-left text-gray-100'>
          {state.connected ? (
            <>
              {items.map((item, i) => {
                return (
                  <div
                    key={i}
                    className={`m-1 p-2 text-xl hover:bg-gray-300 hover:text-gray-800 w-full`}
                    onClick={(i) => {
                      setActive(item.id);
                    }}>
                    {item.name}
