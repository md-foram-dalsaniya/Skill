...
function Profile() {
  ...
  return (
    <div className='flex flex-row h-full w-full p-3 items-center justify-items-stretch text-gray-500'>
      <div className='w-4/6 h-full'>
        <h1 className='text-4xl text-black'><b>{name}</b></h1>
        <div className='text-2xl'>
          <b>{history.location.pathname.includes('user') && role}</b>
        </div>
        <div>
          <h1><b>{location}</b></h1>
        </div>
      ...
      <div className='w-2/6 h-full flex flex-row justify-end'>
        <div className='border-black w-36 h-32 border-solid rounded-full'>
          {/* bg-gray-300 */}
        ...
        <div className={style}>
          <button
            className={
              'bg-red-800 inline text-white active:bg-red-800 font-bold uppercase text-sm px-4 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-0 mb-1 ease-linear transition-all duration-150'
            }
            type='button'
